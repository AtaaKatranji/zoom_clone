		...
		<div {...calendarProps} className="space-y-4 ">
			<div className="relative flex items-center justify-center pt-1">
				<Button
					...
					ref={prevButtonRef}
					variant={"outline"}
					className={cn("absolute left-1 h-7 w-7 bg-transparent bg-dark-1 text-white p-0 opacity-50 hover:opacity-100")}
				>
				...
				<Button
					...
					ref={nextButtonRef}
					variant={"outline"}
					className={cn("absolute right-1 h-7 w-7  p-0 opacity-50 hover:opacity-100")}
				>
...
function CalendarCell({ state, date }: CalendarCellProps) {
	...
	return (
		<td
			...
			className={cn(
				cellProps.className,
				"relative p-0 text-center text-sm focus-within:relative focus-within:z-20 [&:has([aria-selected])]:bg-dark-3 first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md ",
			)}
		>
			<Button
				{...buttonProps}
				...
				className={cn(
					...
					"h-9 w-9",
					isToday ? "bg-dark-2 text-accent-foreground" : "",
					isSelected
						? "bg-dark-2 text-white text-primary-foreground hover:bg-gray-600 hover:text-primary-foreground focus:bg-dark-4 focus:text-primary-foreground"
						: "",
